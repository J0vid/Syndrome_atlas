...
load("data_sparse.Rdata")
 ...
 # save(atlas, tmp.mesh, d.meta.combined, front.face, PC.eigenvectors, synd.lm.coefs, synd.mshape, PC.scores, hdrda.mod, hdrda.df, predshape.lm, file = "data_sparse.Rdata")
 # 
 # atlas <- new_atlas
 # tmp.mesh <- atlas
 # synd.mshape <- d.registered$mshape
 # PC.eigenvectors <- d.registered$PCs[,1:200]
 # PC.scores <- d.registered$PCscores[,1:200]
 ...
 server <- function(input, output, session) {
   ...
   open3d()
   shinySetPar3d(userMatrix = front.face, session = session)
   ...
   output$gestalt <- renderRglwidget({
     par3d(userMatrix = front.face, zoom = .75)
